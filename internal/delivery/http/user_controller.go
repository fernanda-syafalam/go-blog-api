...
func (c *UserController) Register(ctx *fiber.Ctx) error {
	...
	response, err := c.userUseCase.Create(ctx.Context(), request)
	if err != nil {
		c.Log.Warn().Msgf("Failed to create user: %v", err)
		return err
	}

	return utils.SendSuccessResponse(ctx, http.StatusCreated, "Pengguna berhasil dibuat", response)
}
...
func (c *UserController) Login(ctx *fiber.Ctx) error {
	...
	return utils.SendSuccessResponse(ctx, http.StatusOK, "Login berhasil", response)
}
...
func (h *UserController) GetCurrentUser(c *fiber.Ctx) error {
	...
	return utils.SendSuccessResponse(c, http.StatusOK, "Pengguna berhasil diambil", user)
}
...
func (h *UserController) GetAllUsers(c *fiber.Ctx) error {
	...
	return utils.SendSuccessResponse(c, http.StatusOK, "Pengguna berhasil diambil", users)
}

func (h *UserController) GetUserByID(c *fiber.Ctx) error {
	id, err := strconv.ParseUint(c.Params("id"), 10, 32)
	if err != nil {
		return utils.SendErrorResponse(c, http.StatusBadRequest, "ID pengguna tidak valid")
	}
...
func (h *UserController) UpdateUser(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "digunakan") { // Untuk pesan duplikasi
			return utils.SendErrorResponse(c, http.StatusConflict, err.Error())
		}
		return utils.SendErrorResponse(c, http.StatusInternalServerError, err.Error())
	}
	return utils.SendSuccessResponse(c, http.StatusOK, "Pengguna berhasil diperbarui", user)
}
...
func (h *UserController) DeleteUser(c *fiber.Ctx) error {
	...
	return utils.SendSuccessResponse(c, http.StatusNoContent, "Pengguna berhasil dihapus")
}
